...
**Limitations:**
The presented approach shows that GCNs can efficiently perform classification of the squat pose within the graph-based paradigm. However, there exist several limitations, including the following. The performance of the system is strongly determined by the quality of the input videos and the accuracy of MediaPipe Pose landmark detection. The coordinates of the detected skeletal keypoints may depend on the lighting conditions, camera angle, occlusions, subject appearance, and speed of the movement. The assessment of squat quality depends on the fixed SI threshold value that may not cover the variability of human movement patterns or correspond to the complex assessment criteria used by experts. It may be difficult to generalize the trained ST-GCN and CTR-GCN models to other subjects, exercises, and environments due to the small size of the dataset. Moreover, the proposed framework is applicable only for squat movement analysis, which requires development of additional adaptations for other kinds of exercises or rehabilitation movements. The usage of graph-based DL models along with MediaPipe-based features may be challenging for real-time implementation of the system on devices with limited computational resources. The solutions of these problems may include enlargement and diversification of the dataset, adaptation of the labeling, use of several input views, utilization of the ground truth by experts, and design of lightweight models for real-time processing.
